...
ISR(INT5_vect)
{
	...
	_delay_ms(20);		//디바운싱
	while(PINE & 0x20);	//스위치 누름을 기다림
	_delay_ms(20);		//디바운싱
	
	EIFR = 0x20;		//인터럽트 플래그 레지스터 리셋	- 실행후 리셋시켜야함
}
...
ISR(INT6_vect)
{
	...
	EIFR = 0x40;		//인터럽트 플래그 레지스터 리셋	- 실행후 리셋시켜야함
}
...
ISR(INT7_vect)
{
	...
	_delay_ms(20);		//디바운싱
	while(PINE & 0x80);	//스위치 누름을 기다림
	_delay_ms(20);		//디바운싱
	
	EIFR = 0x80;		//인터럽트 플래그 레지스터 리셋	- 실행후 리셋시켜야함
}
...
void init_devices(void)
{
  ...
  DDRE=0x00;	//E 7, 6, 5, 4 입력 설정(인터럽트)
  EICRB = 0xAA;	//인터럽트 7, 6, 5, 4 하강,상승에지 인터럽트 요구
